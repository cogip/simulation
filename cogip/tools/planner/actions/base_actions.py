...
class AlignAction(Action):
    ...
    async def init_poses(self):
        ...
        pose1 = AdaptedPose(
            x=self.start_pose.x,
            y=-1550 + self.game_context.properties.robot_length / 2,
            O=0,
            max_speed_linear=5,
            max_speed_angular=5,
            allow_reverse=True,
    ...
    async def after_pose1(self):
        ...
        if current_pose.x > 0:
            x = 1050 - self.game_context.properties.robot_length / 2
        else:
            x = -1050 + self.game_context.properties.robot_length / 2
        pose3 = AdaptedPose(
            x=x,
...
class DropInDropoffZoneAction(Action):
    ...
    async def after_action(self):
        match self.dropoff_zone.id:
            case artifacts.DropoffZoneID.Top:
                self.actions.append(PushPotAction(self.planner, self.actions, artifacts.PotSupplyID.LocalTop))
            case artifacts.DropoffZoneID.Bottom:
                if self.game_context._table == TableEnum.Training:
                    self.actions.append(PushPotAction(self.planner, self.actions, artifacts.PotSupplyID.LocalMiddle))
