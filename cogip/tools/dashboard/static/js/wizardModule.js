...
function configureChoiceInput(typeInput, value, choices) {
  ...
  setAttributes(choiceZone, { id: "newZone" });

  const inputType = typeInput === "radio" ? "radio" : "checkbox";

  choices.forEach((choice) => {
    const button = createChoiceButton(inputType, choice, value); // Create button for each choice
    const label = createChoiceLabel(choice); // Create label for each choice
...
function createChoiceButton(inputType, choice, value) {
  const isChecked =
  ...
  setAttributes(button, {
    ...
    name: "choice",
    class:
      "w-[15px] h-[15px] rounded-md inline-block bg-white checked:accent-red-cogip",
    value: choice,
    checked: isChecked,
  });
  return button;
...
function createChoiceLabel(choice) {
...
function createCampButton(camp, isActive) {
  ...
  setAttributes(button, {
    type: "button",
    class: `h-[30px] w-[100px] mr-[15px] bg-${cssColor} border-${cssColor} rounded-md ${isActive}`,
    value: camp,
  });
