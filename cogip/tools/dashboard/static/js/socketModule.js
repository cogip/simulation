...
export function onMenu(menu, type, socket) {
  ...
  const otherEntries = menu.entries.filter(
    (entry) => entry.desc !== "Exit Menu"
  );
  ...
  // Add "Exit Menu" first, followed by the sorted entries
  if (exitMenuEntry) {
    sortedEntries.unshift(exitMenuEntry); // Add "Exit Menu" to the beginning
  }
