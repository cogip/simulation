...
class Robot(QtCore.QObject):
    ...
    signal_position_updated = qtSignal(float, float, float)
    ...
    def update_position(self):
        while True:
            (x, y, angle) = self.position_queue.get()
            
            teta = math.radians(float(angle) + 90)

            vector = OCC.Core.gp.gp_Vec(x, y, 0)
            ax = OCC.Core.gp.gp_Ax1(OCC.Core.gp.gp_Pnt(0., 0., 0.), OCC.Core.gp.gp_Dir(0., 0., 1.))
